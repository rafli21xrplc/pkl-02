                ...
                <tbody class="divide-y divide-gray-100 border-t border-gray-100 text-center">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <th class="px-3 py-2 text-center">
                                <div class="sm:col-span-3">
                                    <div class="mt-2">
                                        <select required id="country" name="mahasiswa_id" autocomplete="country-name"
                    ...
                    @foreach ($absens as $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <th class="flex gap-3 px-6 py-4 font-normal text-gray-900 text-left">
                                <div class="relative h-10 w-10">
                                    <img class="h-full w-full rounded-full object-cover object-center"
                                        src="{{ asset('storage/' . $item->mahasiswa[0]->image) }}" alt="" />
                                    <span
                                        class="absolute right-0 bottom-0 h-2 w-2 rounded-full bg-green-400 ring ring-white"></span>
                                </div>
                                <div class="text-sm">
                                    <div class="font-medium text-gray-700">{{ $item->mahasiswa[0]->name }}</div>
                                    <div class="text-gray-400">{{ $item->mahasiswa[0]->email }}</div>
                                </div>
                            </th>
                            ...
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex items-center gap-1 rounded-full text-white px-2 py-1 text-xs font-semibold {{ $item->status == 'Hadir' ? 'bg-green-600' : '' }} {{ $item->status == 'Alpha' ? 'bg-red-600' : '' }} {{ $item->status == 'izin' ? 'bg-yellow-600' : '' }}">
                                    {{ $item->status }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center"> {{ $item->date }}
                            </td>
                            <td class="px-6 py-4 text-center">surat</td>
                            <td class="px-6 py-4">
                                <div class="flex justify-center gap-4 ">
                                    <a x-data="{ tooltip: 'Delete' }" href="/deleted/{{ $item->code }}">
